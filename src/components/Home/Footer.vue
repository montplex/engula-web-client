<template>
	<footer class="ly-box pb-0 hidden">
		<div class="sunset-right"></div>
		<div class="top-rect"></div>
		<div class="content">
			<div class="home-footer pb-12 pt-12">
				<!-- <img class="w-44 h-10" src="@/assets/images/logo.png" alt="MontPlex" /> -->
				<div class="flex items-center mt-6">
					<img width="54" src="@/assets/images/main-logo.png" alt="" />
					<div class="flex items-center pl-2 font-semibold text-5xl">
						<span class="text-block">Montplex</span>
					</div>
				</div>

				<div class="meta-walk md:py-14 py-8">
					<a href="#">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							width="1em"
							height="1em"
							aria-hidden="true"
							focusable="false"
							class="text-white text-2xl"
						>
							<path
								fill="currentColor"
								d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"
							></path>
						</svg>
					</a>
					<a href="#">
						<svg
							viewBox="0 0 24 24"
							xmlns="http://www.w3.org/2000/svg"
							width="1em"
							height="1em"
							aria-hidden="true"
							focusable="false"
							class="text-white text-2xl"
						>
							<path
								fill="currentColor"
								d="M13.54 12a6.8 6.8 0 01-6.77 6.82A6.8 6.8 0 010 12a6.8 6.8 0 016.77-6.82A6.8 6.8 0 0113.54 12zm7.42 0c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75-.66 0-1.19-2.58-1.19-5.75s.53-5.75 1.19-5.75C23.47 6.25 24 8.83 24 12z"
							></path>
						</svg>
					</a>
					<a href="#">
						<svg
							role="img"
							width="1em"
							height="1em"
							class="text-white text-2xl"
							viewBox="0 0 24 24"
							xmlns="http://www.w3.org/2000/svg"
						>
							<path
								fill="currentColor"
								d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"
							></path>
						</svg>
					</a>
				</div>
				<ul class="quick-list text-xl">
					<li>
						<a
							class="flex"
							href="https://dev.montplex.com/docs/docs/support/contact-us"
							target="_blank"
						>
							<span class="text-block">{{ $t("home.contact") }}</span>
							<svgIcon icon="right45" class="!h-5 !w-5 -rotate-45" />
						</a>
					</li>

					<li class="flex" @click="handlePrivacy('policy')">
						<span class="text-block">{{
							$t("home.footer.privacyPolicy")
						}}</span>
						<svgIcon icon="right45" class="!h-5 !w-5 -rotate-45" />
					</li>

					<li class="flex" @click="handlePrivacy('service')">
						<span class="text-block">{{
							$t("home.footer.termsofService")
						}}</span>
						<svgIcon icon="right45" class="!h-5 !w-5 -rotate-45" />
					</li>
				</ul>
				<div class="md:my-10 py-6 text-xl text-white">
					© 2022 Montplex, Inc. All rights reserved
				</div>
				<div
					class="hidden px-40 text-center text-sm md:block font-light"
					:style="{
						color: 'rgb(255 255 255 / 36%)',
						fontFamily: 'Airbnbcereal book'
					}"
				>
					<p class="max-w-2xl" v-if="locale === 'en'">
						Redis is a trademark of Redis Labs Ltd. Any rights therein are
						reserved to Redis Ltd. Any use by Montplex is for referential
						purposes only and does not indicate any sponsorship, endorsement or
						affiliation between Redis and Montplex.
					</p>
				</div>
			</div>
		</div>
	</footer>

	<footer class="container mx-auto">
		<div class="gradient-border division-line" style="padding: 1px"></div>
		<div class="flex items-center py-8">
			<!-- <div class="flex items-center">
				<img width="24" src="@/assets/images/main-logo.png" alt="" />
				<div class="flex items-center pl-1 font-semibold text-2xl">
					<span class="text-block">Montplex</span>
				</div>
			</div> -->
			<div class="flex-1 text-white">© 2022 Montplex, Inc.</div>
			<div class="flex items-center gap-6 meta-walk-svg text-white">
				<a href="#">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="1em"
						height="1em"
						aria-hidden="true"
						focusable="false"
						class="text-2xl"
					>
						<path
							fill="currentColor"
							d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"
						></path>
					</svg>
				</a>
				<a href="#">
					<svg
						viewBox="0 0 24 24"
						xmlns="http://www.w3.org/2000/svg"
						width="1em"
						height="1em"
						aria-hidden="true"
						focusable="false"
						class="text-2xl"
					>
						<path
							fill="currentColor"
							d="M13.54 12a6.8 6.8 0 01-6.77 6.82A6.8 6.8 0 010 12a6.8 6.8 0 016.77-6.82A6.8 6.8 0 0113.54 12zm7.42 0c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75-.66 0-1.19-2.58-1.19-5.75s.53-5.75 1.19-5.75C23.47 6.25 24 8.83 24 12z"
						></path>
					</svg>
				</a>
				<a href="#">
					<svg
						role="img"
						width="1em"
						height="1em"
						class="text-2xl"
						viewBox="0 0 24 24"
						xmlns="http://www.w3.org/2000/svg"
					>
						<path
							fill="currentColor"
							d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"
						></path>
					</svg>
				</a>
			</div>
		</div>
	</footer>
</template>

<script setup lang="ts">
import { useI18n } from "vue-i18n";
import { useRouter } from "vue-router";
const { locale } = useI18n();
const router = useRouter();

// 访问隐私和权限
const handlePrivacy = (str: string) => {
	router.push({ path: "/pdf", query: { type: str } });
};

/* function handlePrivacy(str: string) {
	router.push({ path: "/pdf", query: { type: str } });
} */
</script>

<style lang="scss" scoped>
.home-footer {
	display: flex;
	flex-direction: column;
	align-items: center;
	.meta-walk {
		display: flex;
		align-items: center;
		a {
			display: flex;
			justify-content: center;
			align-items: center;
			min-width: 4rem;
			height: 4rem;
			background-color: rgb(255 255 255 / 0.08);
			border-radius: 100%;
			&:hover {
				background-color: rgb(255 255 255 / 0.16);
			}
			&:not(:last-of-type) {
				margin-right: 1rem;
			}
		}
	}
}

.quick-list {
	@apply flex items-center flex-col md:flex-row gap-12;
	li,
	li a {
		@apply flex items-center gap-1 cursor-pointer;
		span {
			border-bottom: 1px dashed transparent;
		}

		&:hover {
			span {
				border-color: #2ab5fb;
			}
		}
	}
}

.meta-walk-svg {
	a {
		display: flex;
		justify-content: center;
		align-items: center;
		min-width: 3rem;
		height: 3rem;
		// background-color: rgb(255 255 255 / 0.08);
		border-radius: 100%;
		&:hover {
			background-color: rgb(255 255 255 / 0.16);
		}
	}
}
</style>
